...
class Parser:
    ...
    # is the next tokenType what we think it is?
    ...
    def __parse_expression_statement(self) -> ExpressionStatement:
        expression = self.__parse_expression(PrecedenceType.P_LOWEST) # root of pratt-parser

        if self.__peek_token_is(TokenType.SEMICOLON):
            self.__next_token()

        statement: ExpressionStatement = ExpressionStatement(expression=expression)
        return statement  # Missing in your code
    

    def __parse_let_statement(self) -> LetStatement:
        # we are trying to parse this:
        # Let a: int = 10;
        ...
        # We know for sure we have an identifier at this point
        ...
        # We know for sure we a TYPE token here
        statement.value_type = self.current_token.literal

        # I guess optional would go somewhere here?
    ...
    def __parse_function_statement(self) -> FunctionStatement:
        ...
        # to parse: fn name() -> int { return 10 }
